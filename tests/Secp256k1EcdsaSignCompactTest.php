...
class Secp256k1EcdsaSignCompactTest extends TestCase
{
    public function testSignCompact()
    {
        ...
        $this->assertEquals(1, secp256k1_ecdsa_sign_compact(TestCase::getContext(), $msg, $key, $sig, $recid));
    ...
    public function testRandomSign()
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            $publicKey = '';
            $this->assertEquals(1, secp256k1_ec_pubkey_create($context, $private, $compressed, $publicKey));

            $msg = hash('sha256', $i, true);
            $sig = '';
            $recid = 0;
            $this->assertEquals(1, secp256k1_ecdsa_sign_compact($context, $msg, $private, $sig, $recid));

            $byte = 27 + $recid + ($compressed ? 4 : 0);
            $byteHex = str_pad(dechex($byte), 2, '0', STR_PAD_LEFT);
            $bin = pack("H*", $byteHex) . $sig;
    ...
    public function decodeTest($context, $base64, $hash, $expectedRecid, $expectedCompressed, $expectedPubkeyBS)
    {
        ...
        $publicKey = '';
        $this->assertEquals(1, secp256k1_ecdsa_recover_compact($context, $hash, $sig, $recid, $compressed, $publicKey));
